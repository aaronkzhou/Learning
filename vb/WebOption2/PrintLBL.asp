<%@ Language=VBScript %>
<%
	Dim dicData,strU_PN,strAction,strCOO,strDescription,strAgent,strAddress,strPhone,strC_PN
	...
	if strAction = "ShowData" then
		...
		strQuery1 = "select * from Option_COO where COO='" & strC_PN & "'"
...
<%  		
 	if strAction = "ShowData" then
		...
		dbRS.MoveFirst
		do while NOT dbRS.EOF
			response.write("<TR><TD borderColor=lightsteelblue><font face='Futura Bk' size='2'>" & strU_PN & "</font></TD>" & vbCRLF)
			response.write("<TD borderColor=lightsteelblue><font face='Futura Bk' size='2'>" & trim(dbRS.fields("Description")) & "</font></TD>" & vbCRLF)
			response.write("<TD borderColor=lightsteelblue><font face='Futura Bk' size='2'>" & strC_PN & "</font></TD>" & vbCRLF)
			response.write("<TD borderColor=lightsteelblue><font face='Futura Bk' size='2'>" & strCOO & "</font></TD></TR>" & vbCRLF)
			dbRS.MoveNext
			count = count + 1
